...
long ParseInputFiles(std::vector<CFastaFile*>* pvecFastaFilesSource, std::vector<CTable*>* pvecOrthoGroups, size_t* psizeOrthoGroups, std::vector<std::string*>* pvecOrthoGroupNamesFromSetOneAndTwo, std::vector<std::string*>* pvecOrthoGroupNamesFromSetOneAndTwoFiltered,bool bUsePPPInput);
...
int main(int argc, char* argv[], char* envp[])
{
	...
	bool bUsePPPInput = false;
    lReturn = glb.propertyBag.GetBoolPPPInputGrab(&bUsePPPInput);
	CHECK_RETURNVALUE(lReturn);
    ...
    //STEP TWO
    //Parse Input Files
    lReturn = ParseInputFiles(&vecFastaFileSource, &vecOrthogroups, &sizeOrthoGroups, &vecOrthoGroupNamesFromSetOneAndTwo, &vecOrthoGroupNamesFromSetOneAndTwoFiltered,bUsePPPInput);
    ...
    {
        printf_s("Copy the PhyloPypruner input files based on the filtered set to the output folder %s", EOL);
        lReturn = PhyloPyPrunerSourcesProcess(&vecOrthoGroupNamesFromSetOneAndTwoFiltered);
		CheckReturnValue(lReturn, &vecFastaFileSource, &vecFastaFileOrthoGroups, &vecFastaFileOrthoGroupsFiltered, &vecOrthogroups, &vecOrthoGroupNamesFromSetOneAndTwo, &vecOrthoGroupNamesFromSetOneAndTwoFiltered);
        CHECK_RETURNVALUE(lReturn);
        printf_s("Done with copying %s", EOL);
        //New Function that grabs and moves
    }
