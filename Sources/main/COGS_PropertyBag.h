...
public:
	...
	*/
	...
	long SetBoolPPPInputGrab(bool bSetPPPInputGrab = true)
	{
		return SetValueBool("Bool", "PPPInput", bSetPPPInputGrab);
	}

	long GetBoolPPPInputGrab(bool* pGSetPPPInputGrab)
	{
		long lReturn = ERR_NOERROR;
		lReturn = GetValueBool("Bool", "PPPInput", pGSetPPPInputGrab, false);
		if (lReturn == PROP_ERR_SECTION_NOT_FOUND_DEFAULT_USED || lReturn == PROP_ERR_KEY_NOT_FOUND_DEFAULT_USED)
		{
			printf_s("%s %s", "pGSetPPPInputGrab Bool Not Found: Default false used", EOL);
			return ERR_NOERROR;
		}
		return lReturn;
	}

	long SetPPPInputDirSet1(const char* szPPPInputDirSet1)
	{
		std::string strPPPInputDir1 = szPPPInputDirSet1;
		AppendSlash(&strPPPInputDir1); //This is to make sure the path ends with a /
		return SetValueString("Paths", "PPPDir1", strPPPInputDir1.c_str());
	}

	long GetPPPInputDirSet1(std::string* pstrPPPInputDir1)
	{
		return GetValueString("Paths", "PPPDir1", pstrPPPInputDir1);
	}

	long SetPPPInputDirSet2(const char* szPPPInputDirSet2)
	{
		std::string strPPPInputDir2 = szPPPInputDirSet2;
		AppendSlash(&strPPPInputDir2); //This is to make sure the path ends with a /
		return SetValueString("Paths", "PPPDir2", strPPPInputDir2.c_str());
	}

	long GetPPPInputDirSet2(std::string* pstrPPPInputDir2)
	{
		return GetValueString("Paths", "PPPDir2", pstrPPPInputDir2);
	}
