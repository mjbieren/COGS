	...
	for (int i = 1; i < argc; i++)
	{
		...
		if (*argv[i] == '-') //switch detected
		{
			...
			case 'p': //PhyloPyPrunerFormat
				...
				aiRequirements[5] = 1;
				break;
			case 'x': //PPP Input Set 1 and option to use this as source
				lRet = glb.propertyBag.SetBoolPPPInputGrab();
				if (lRet != ERR_NOERROR)
					return PARSECOMMANDLINE_PPPINPUTGRAB_COGS;
				lRet = glb.propertyBag.SetPPPInputDirSet1(&argv[i][3]);;
				if (lRet != ERR_NOERROR)
					return PARSECOMMANDLINE_PPPINPUTDIRONE_COGS;
				aiRequirements[6] = 1;
				break;
			case 'y': //PPP Input Set 1 and option to use this as source
				lRet = glb.propertyBag.SetBoolPPPInputGrab();
				if (lRet != ERR_NOERROR)
					return PARSECOMMANDLINE_PPPINPUTGRAB_COGS;
				lRet = glb.propertyBag.SetPPPInputDirSet2(&argv[i][3]);;
				if (lRet != ERR_NOERROR)
					return PARSECOMMANDLINE_PPPINPUTDIRTWO_COGS;
				aiRequirements[7] = 1;
				break;
			default:
			{
...
*/
		...
		else if (*argv[i] == '?')
		{
			...
			printf_s(
				...
				"-x <PPPInputDirSet1> \t\t Use Input directory of Set one for PhyloPyPruner instead of the base file. %s"
				"-y <PPPInputDirSet2> \t\t Use Input directory of Set two for PhyloPyPruner instead of the base file. %s"
				"%s"
	...
	if (aiRequirements[6] == 1 && aiRequirements[7] == 1)
	{
		aiRequirements[0] = 1;
		aiRequirements[3] = 1;
	}
